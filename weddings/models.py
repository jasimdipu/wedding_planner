...
class Event(models.Model):
    ...
    wedding_id = models.ForeignKey("Wedding", null=True, blank=True, on_delete=models.SET_NULL)
...
class ProductIncluded(models.Model):
    event_id = models.ForeignKey(Event, null=True, blank=True, on_delete=models.SET_NULL)
    provides_product_id = models.ForeignKey(
        "ProvidesProduct", null=True, blank=True, on_delete=models.SET_NULL)
    price = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True)
    current_status_id = models.ForeignKey("Status", null=True, blank=True, on_delete=models.SET_NULL)

    def __str__(self) -> str:
        return self.event_id

class ProvidesProduct(models.Model):
    partner_id = models.ForeignKey(Partner, null=True, blank=True, on_delete=models.SET_NULL)
